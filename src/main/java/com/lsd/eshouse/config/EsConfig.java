...
public class EsConfig {
    ...
    @Value("#{'${eshouse.elasticsearch.node-list}'.split(',')}")
    ...
    public RestClientBuilder restClientBuilder() {
        ...
        var builder = RestClient.builder(this.getHttpHostList(nodeList));
    ...
    public RestClient restClient() {
        // 连接失败时的嗅探监听器
        var sniffOnFailureListener = new SniffOnFailureListener();
        ...
        var sniffer = Sniffer.builder(restClient)
    ...
    public RestHighLevelClient restHighLevelClient() {
        ...
        var listener = new SniffOnFailureListener();
        builder.setFailureListener(listener);
        var client = new RestHighLevelClient(builder);
        var sniffer = Sniffer.builder(client.getLowLevelClient())
                .setSniffIntervalMillis(60000)              //常规嗅探60s一次，用于更新节点信息。默认5分钟
                .setSniffAfterFailureDelayMillis(30000)     //连接失败后开启额外的嗅探task30s一次，默认1分钟
                .build();
